...
| Acelerômetro / giroscópio / magnetômetro | 🟡 | Despriorizado para servidor headless (em espera / backlog). Roteados internamente ao Qualcomm Hexagon ADSP (não SLPI). Pilha QMI confirmada via QRTR (`qrtr-lookup` detecta `SNS_SMGR` no port 256). FastRPC in-kernel dispensado. Leitura de dados documentada em `docs/FASTRPC_AND_SENSORS_SUBSYSTEM_PLAN.md`. |
