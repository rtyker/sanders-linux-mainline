...
| Botões físicos (Volume Up/Down, Power) | ✅ | **Confirmado ao vivo em 2026-09-04** (`docs/archived/AUDIO_VOLUME_BUTTONS_INVESTIGATION.md`). Volume Up (`gpio-keys`/`event4`, `KEY_VOLUMEUP`) precisava do pinctrl `gpio_key_default` em `&tlmm` (faltava no DTS). Volume Down (`pm8941_resin`/`event1`, `KEY_VOLUMEDOWN`) e Power (`pm8941_pwrkey`/`event0`, `KEY_POWER`) dependem do driver PON (`CONFIG_POWER_RESET_QCOM_PON=y`) — sem ele `pon@800` ficava `deferred` e os child nodes nunca eram criados. Eventos de press/release capturados limpos nos três; Power já confirmado disparando o `poweroff` padrão do systemd-logind (cuidado: um toque desliga o aparelho de verdade). Pendente: nenhum daemon ainda reage a KEY_VOLUMEUP/DOWN pra mudar volume real do ALSA/PipeWire. |
...
| Áudio | ✅ | **Validado ao vivo em 2026-09-04:** Qualcomm Hexagon ADSP + SPMI WCD analog codec funcional, placa ALSA `motorolapotter` e servidor PipeWire/WirePlumber com `pulsesink`. Reprodução de áudio MP3 audível no alto-falante físico, ganho analógico calibrado para 75 (-9 dB) e controle de volume integrado às teclas físicas laterais (`sanders-volume-keys.service`) e aos controles de toque da ClickWheel. |
